...
# Add HAZM for persian preprocess
import hazm
#################################

class Corpus(Dataset):
    def __init__(self, path, word_dic=None, min_word_count=4, sort=True, is_digikala=False):
        ...
        if is_digikala == True:
          ...
          lem = hazm.Lemmatizer()
          stem = hazm.Stemmer()
          # Remove sentences longer than 30 words (Colab cannot handle it man)
          corpus = [([stem.stem(lem.lemmatize(word)) for word in hazm.word_tokenize(norm.normalize(sentence))], len(hazm.word_tokenize(norm.normalize(sentence)))) for sentence in corpus if len(hazm.word_tokenize(norm.normalize(sentence))) <= 15]
        ###########################################

        else:
          corpus = [self._split(i.strip()) for i in corpus]
          corpus = [(t, len(t)) for t in corpus if len(t) <= 30]
