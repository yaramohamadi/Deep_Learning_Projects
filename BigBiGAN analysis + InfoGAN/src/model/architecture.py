...
class GAN(torch.nn.Module):
    ...
    # InfoGAN: Add c_info ##################################################
    def generate_imgs(self, cls=None, noise=None, fixed=False, c_info=False):
    ########################################################################
        ...
        # InfoGAN: Input c_info to generator ##############################
        img_gen = self.generator(z=noise, cls=cls, c_info=c_info)
        ###################################################################
...
class BigBiGAN(GAN):
    ...
    def forward(self, img_real, img_gen, z_noise, z_img, cls):
        ...
        # InfoGAN: Get C output from image discriminator ####################################
        img_real_out, img_real_score, c_real_predict = self.img_discriminator(x=img_real + noise, cls=cls)
        img_gen_out, img_gen_score, c_gen_predict = self.img_discriminator(x=img_gen + noise, cls=cls)
        #####################################################################################
        ...
        output = {
            ...
            # InfoGAN: output C predictions #####################
            "c_real_predict": c_real_predict,
            "c_gen_predict": c_gen_predict
            #####################################################
        }
