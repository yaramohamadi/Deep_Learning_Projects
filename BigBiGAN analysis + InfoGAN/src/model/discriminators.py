...
class DiscBigGAN(nn.Module):
    def __init__(self, mult_chs, ks, num_cls, sn, w_init):
        ...
        # Infogan: Add softmax and linear layer###########################
        self.infogan_linear = nn.Linear(in_features=resblocks_output, out_features=128)
        self.infogan_leakyRelu = nn.LeakyReLU(negative_slope=0.1)
        self.infogan_linear2 = nn.Linear(in_features=128, out_features=10)
        self.infogan_softmax = nn.Softmax(dim=1)
        ##################################################################
    ...
    def forward(self, x, cls):
        ...
        # InfoGAN: Create C output###########
        y3 = self.infogan_linear(y1)
        y3 = self.infogan_leakyRelu(y3)
        y3 = self.infogan_linear2(y3)
        y3 = self.infogan_softmax(y3)
        #####################################
